...
public class GOTBlockCraftingTable extends Block {
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
        boolean hasRequiredAlignment = GOTLevelData.getData(entityplayer).getAlignment(this.tableFaction) > 99.0;
        if (hasRequiredAlignment) {
            if (!world.isRemote) {
                entityplayer.openGui(GOT.instance, this.tableGUIID, world, i, j, k);
            }
        } else {
            for (int l = 0; l < 8; ++l) {
            ...
            if (!world.isRemote) {
                GOTAlignmentValues.notifyAlignmentNotHighEnough(entityplayer, 100.0f, this.tableFaction);
            }
        }
        return true;
    }
